...
export default function EditComponent({ component }: { component: Component }) {
    ...
    const [twConfig, setTwConfig] = useState(component.tailwindConfig ? JSON.stringify(JSON.parse(component.tailwindConfig), null, 2) : "");
    ...
    async function updateComponent() {
        ...
        router.push(`/contribute/edit-draft-documentation/${componentToUpload.uid ?? componentToUpload.id}`);
    }
    ...
    return <div className="flex flex-col h-full w-full items-start pt-4">
        ...
        <div className="flex flex-col w-full gap-5 mt-3 max-w-[55rem]">
            <div className="flex flex-col gap-1.5">
                ...
                <Input placeholder="button" disabled={true} value={componentId} onChange={onComponentIdChange} />
                ...
                {
                    !nameValidated &&
                    <Button variant="default" className="mt-3" disabled={analysing} onClick={checkNameValidity}>
            ...
            {nameValidated && <div className="flex flex-col gap-1.5">
                ...
                <Textarea value={code} rows={14} spellCheck={false} className={cn(robotoMono.className)} onChange={onCodeChange} placeholder={placeHolderComponent} />
                ...
                {
                    !codeIsValid &&
                    <Button variant="default" className="mt-3" disabled={analysing} onClick={analyseCode}>
                        {analysing && <Loader2 className="w-6 h-6 mr-4 animate-spin" />}
                        Analyse Source Code
                    </Button>}
                {
                    codeIsValid && <>
                        ...
                        {!dependancyConfirmed && <Button variant="default" className="mt-3" disabled={analysing} onClick={setDependancyVersions}>
                            {analysing && <Loader2 className="w-6 h-6 mr-4 animate-spin" />}
                        ...
                        {dependancyConfirmed && <><div className="flex flex-col gap-1.5 mt-6">
                            ...
                            <Textarea value={twConfig} rows={10} id="additional-tailwind-config" onChange={onConfigChange} spellCheck={false} className={cn(robotoMono.className)} placeholder={placeHolderConfig} />
                        </div>
                            {
                                !configValidated &&
                                <Button variant="default" className="mt-3" disabled={analysing} onClick={validateConfig}>
                                    {analysing && <Loader2 className="w-6 h-6 mr-4 animate-spin" />}
                            ...
                            }
                            {
                                ...
                                <>
                                    ...
                                    <Input value={fileLocation} placeholder="components/ui/Button.tsx" onChange={onFileLocationChange} />
                                    <span className="text-sm text-gray-400">The location to which the file will be copied. Must belong to either components/ or lib/ directory.</span>
                                    {
                                        ...
                                        <Button variant="default" className="mt-3" disabled={analysing} onClick={() => {
                                            setAnalysing(true);
                                    ...
                                    {
                                        fileLocationValidated &&
                                        <Button variant="default" className="mt-3" onClick={updateComponent} disabled={analysing}>
                                            {analysing && <Loader2 className="w-6 h-6 mr-4 animate-spin" />}
                                            Apply Edits
                                        </Button>
                                    }
...
const buttonVariants = cva(
    ...
    variants: {
        ...
        default:
            "bg-default text-default-foreground hover:bg-default/80",
